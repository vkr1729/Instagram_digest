...
- [ ] **UAT-3.2 Guaranteed Representation**: Every active creator who posted at least 1 valid reel in the 7-day window receives at least 1 slot in the Top 300.
- [ ] **UAT-3.3 Creator Saturation Cap**: No creator can occupy more than 4 slots in the Top 300 digest, preventing feed takeover.
- [ ] **UAT-3.4 Deterministic Output**: The ranked output produces exactly up to 300 items (or all candidates if <300), numbered #01 through #300 in descending score order, output to `data/top100_digest.json`.
...
- [ ] **UAT-4.3 8-Day Rolling Purge (R2 Bucket)**: Scans objects in `videos/` on R2; automatically deletes all video files older than 8 days.
- [ ] **UAT-4.4 8-Day Rolling Purge (Local Disk)**: Purges local video downloads older than 8 days from `~/.instagram_digest/videos/`.
...
- [ ] **UAT-5.4 5 Story Category Circles**: Renders 5 gradient-ringed circles: `🔥 All Top 300`, `💻 Tech & AI`, `🏋️ Health & Wellness`, `🧠 Deep Explainer`, `🎨 Creative & Culture`.
...
- [ ] **UAT-6.2 Story Category Filter**: Clicking `💻 Tech & AI` story circle immediately filters visible cards to only Tech creators; clicking `🔥 All Top 300` restores all 300.
- [ ] **UAT-6.3 Default Playback Speed**: Verifies initial `video.playbackRate === 1.25` from `DEFAULT_PLAYBACK_SPEED` (no header speed pill exists).
