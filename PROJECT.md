...
- [ACCEPTED FOLLOW-UP F1] Fast-scroll autoplay race fix (user authorized 2026-09-11: "make the necessary fixes"). Fixed two real defects in `templates/partials/player.js` `playCardVideo()`: (a) neighbor reset set `currentTime = 0` at HAVE_NOTHING (throws InvalidStateError); (b) any play() rejection incl. AbortError hit the muted fallback, replaying stale cards. Fix: `readyState > 0` guard, shared `myGen`/`navGen` token, AbortError bypass. Covered by `tests/test_playback_race.py`. Implemented 2026-09-11.
- [CORRECTION 2026-09-11] F1 was NOT the reported field cause: the no-autoplay symptom persisted with a regular every-2nd-or-3rd-reel cadence, which a sporadic throw cannot explain. The (a)/(b) defects were real but not the field bug. Live diagnosis opened via `?mediadebug=1` on-screen tracer (`mtrace` ring buffer in `player.js`; zero impact when absent) + waiter-token cleanup on error/src-detach. Awaiting field trace (screenshot) to identify the true cause.
- [iOS TAP-RESUME LOOP 2026-09-11] Desktop field trace exonerated tap logic: two discrete taps 382ms apart, each handled correctly (pause, resume); autoplay NotAllowedError→muted fallback verified normal. iOS hypothesis: Safari scrolls tapped content into view, re-driving observer/settle autoplay and undoing tap-pauses (desktop Chrome never scrolls on tap). Fix: 1500ms `manualPause` cooldown in `playCardVideo` — auto paths blocked, tap resume bypasses via `isManual`, cleared on card change. Pending iPhone field verification (?mediadebug=1 in iOS Safari).
- [CAUGHT 2026-09-11] iPhone trace nailed the skip: reel 6 `play-reject NotAllowedError` at rs=4, muted fallback also failed with zero observable output, reel dead 24s until scroll (shot 3). Reels 3–5 auto-played cleanly ~1s apart (shot 2); tap pause/resume correct (shot 1). Likely trigger: Low Power Mode (yellow battery in shot) + expired tap-activation → iOS rejects programmatic play. Fix: fallback failure now traced (`play-reject2`) and shows the ▶ tap affordance (tap already resumes via manual path). Pending: one session with Low Power Mode OFF to confirm the trigger.
 - [CONFIRMED 2026-09-11] Low Power Mode OFF session: scroll-pause and dead-reel issues gone; first-tap stall traced to tap-pause double-registration (fixed by the 1500ms cooldown path). New find: tap-to-unmute paused instead of unmuting — touchstart restorer flipped sound + hid the pill mid-gesture, click retargeted to video → pause. Fix: `gestureUnmuteAt` stamp consumed by card click, tap timer swallows same-gesture pause on playing video (`tests/test_tap_unmute.py`). Mid-scroll muted reels after 3–4 reels need deliberate tap = genuine iOS audio policy (only mute path is the play-reject fallback); left as designed, no workaround exists.
...
## Desktop dashboard (Final — accepted 2026-09-12)

Dedicated desktop controls page; the reel viewer stays mobile-first.
- [SETTLED D1] Form (interview 2026-09-12, answer A): new standalone local-only `/dashboard` page served by `local_server.py` (same pattern as `/channels` → `templates/channels.html`), never deployed to GitHub Pages. It becomes the single home for desktop controls; rejected: (b) growing the viewer header further, (c) a separate app/repo.
- [SETTLED D2] v1 panel scope (interview 2026-09-12, answer A): operations core — run buttons (weekly sync, +100 with count, ad-hoc, cookie refresh, retrigger) + live pipeline status + checkpoint/resume state (needs one tiny new status API; checkpoints are disk-only today) + nav links to viewer/channels. Deferred: log tail viewer, digest stats/history. Rejected: (b) everything-in-v1, (c) button relocation with no new surfacing.
- [SETTLED D3] viewer header relationship (interview 2026-09-12, answer A): strip the desktop ops buttons (+100, ad-hoc refresh, cookie refresh) from the viewer header entirely; viewer goes pure-viewing. No transition duplication.
- [SETTLED D4] landing + nav (user ask 2026-09-12): on desktop the local root (`/`) lands on the dashboard; the dashboard links to the viewer; mobile UA (and the installed PWA) still land on the viewer. Explicit `/viewer` and `/dashboard` paths always work.
- [AMENDED D4, 2026-09-12 — UA sniffing dropped per user call, agreed]: no sniffing. Root (`/`) keeps serving the viewer everywhere (zero PWA risk, fully deterministic, trivially testable); `/dashboard` is the ops page with `/viewer` as its alias; the desktop launcher is retargeted to open `/dashboard` instead of root. The "desktop lands on dashboard" outcome is delivered by the launcher entry point, not by guessing the client.
- [SETTLED D5] viewer consistency (user ask 2026-09-12): viewer header goes identical across desktop and mobile, EXCEPT the offline download button stays everywhere (heavily used on mobile). Dashboard link in the viewer is local-only (no dashboard exists in the deployed Pages build).
- [AGENT DEFAULT, reversible] Tech: raw static `templates/dashboard.html` served directly by `local_server.py` (same pattern as `/channels`), polling the existing status APIs plus one tiny new resume-state API. Follows repo precedent; no Jinja/build dependency for ops UI.
- Scope contract ACCEPTED by owner (chat reply "accept", 2026-09-12): artifact boundary, done-means (a)–(e), design-only approval, no scope widening via execution words. Section status: Final. Implementation NOT authorized — builds only on a separate explicit request.
